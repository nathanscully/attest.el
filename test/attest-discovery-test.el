...
(ert-deftest attest-discovery-cache-evicts-least-recently-used-entry ()
  "The shared discovery cache stays within its configured entry bound."
  (let ((attest-max-position-cache-entries 2)
        (attest--position-cache (make-hash-table :test 'equal))
        (attest--position-cache-used (make-hash-table :test 'equal))
        (attest--position-cache-clock 0))
    (attest--position-cache-store "a" 'a '())
    (attest--position-cache-store "b" 'b '())
    (attest--position-cache-touch "a")
    (attest--position-cache-store "c" 'c '())
    (should (= 2 (hash-table-count attest--position-cache)))
    (should (gethash "a" attest--position-cache))
    (should-not (gethash "b" attest--position-cache))
    (should (gethash "c" attest--position-cache))))

(ert-deftest attest-discovery-cache-revision-invalidates-provider ()
  "Replacing a backend registration forces a fresh discovery snapshot."
  (skip-unless (treesit-language-available-p 'typescript))
  (let* ((file (attest-test-fixture "demo.test.ts"))
         (attest--position-cache (make-hash-table :test 'equal))
         (attest--position-cache-used (make-hash-table :test 'equal))
         (attest--backend-revisions (copy-hash-table attest--backend-revisions))
         (attest--backends (copy-tree attest--backends))
         (props (copy-sequence (attest-backend-props 'node)))
         (calls 0))
    (attest-file-positions file 'node)
    (cl-letf* ((orig (symbol-function 'attest--buffer-positions))
               ((symbol-function 'attest--buffer-positions)
                (lambda (&rest args)
                  (cl-incf calls)
                  (apply orig args))))
      (apply #'attest-register-backend 'node props)
      (attest-file-positions file 'node))
    (should (= 1 calls))))
