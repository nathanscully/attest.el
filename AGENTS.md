...
3. `neotest.el`: the core, 660 lines. Everything else is a backend or a
   consumer of it.
...
On this machine pytest is not on PATH. Run the suite as
`nix shell nixpkgs#python3Packages.pytest -c make all`, or point
`neotest-pytest-command` at a venv.
